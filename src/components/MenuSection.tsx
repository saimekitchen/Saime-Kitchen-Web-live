...
export default function MenuSection({
  ...
  const [selectedCategory, setSelectedCategory] = useState<string>('Healthy Diet');
  ...
  const [customCategories, setCustomCategories] = useState<{ value: string; menuType: 'bistro' | 'cafe' | 'cocktail' }[]>(() => {
    ...
    if (saved) {
      try {
        ...
        // Inject Healthy diet category if missing
        const hasHealthy = parsed.some((c: any) => c.menuType === 'bistro' && (c.value === 'Healthy diet' || c.value === 'Healthy Diet'));
        if (!hasHealthy) {
          parsed.push({ value: 'Healthy diet', menuType: 'bistro' });
        }
    ...
    return [
      ...
      { value: 'Healthy diet', menuType: 'bistro' },
  ...
  const [categoryMapping, setCategoryMapping] = useState<Record<string, string>>(() => {
    ...
    if (saved) {
      try {
        ...
        // Inject Healthy diet category mapping if missing
        if (!parsed['Healthy diet'] || !parsed['Healthy Diet']) {
          parsed['Healthy diet'] = 'Healthy Diet';
          parsed['Healthy Diet'] = 'Healthy Diet';
        }
  ...
  // Reset and remove vegetarian filter if they browse to Cafe or Cocktail (Bar)
  useEffect(() => {
  ...
  const categoryMap = useMemo(() => {
    if (currentMenuTab === 'bistro') {
      return [
        { value: 'Healthy Diet', labelEn: 'Healthy Diet', labelVn: 'Chế Độ Ăn Lành Mạnh' },
