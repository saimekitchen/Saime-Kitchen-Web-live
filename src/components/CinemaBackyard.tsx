  ...
  },
  {
    ...
    sessions: [
      ...
      },
      {
        ...
        image: 'https://www.pluggedin.com/wp-content/uploads/2019/12/the-pursuit-of-happyness-1024x682.jpg',
        rating: '8.0/10',
        timeEn: 'Right after Session 1',
        timeVn: 'Ngay sau Suất 1'
      }
...
export default function CinemaBackyard({ lang, onNavigate }: CinemaBackyardProps) {
  ...
  const handleDownloadPNG = async () => {
    ...
    const imagePromises = moviesList.flatMap(m =>
      m.sessions.map(async (sess) => {
        const img = await loadImage(sess.image);
        return { url: sess.image, img };
      })
    );
    const loadedImageList = await Promise.all(imagePromises);
    ...
    ctx.fillRect(0, 0, 1125, 417);

    // Draw an outer border in amber/gold
    ctx.strokeStyle = '#d97706'; // amber-600
    ctx.lineWidth = 4;
    ctx.strokeRect(8, 8, 1109, 401);

    // Draw inner thin accent line
    ctx.strokeStyle = 'rgba(217, 119, 6, 0.25)';
    ctx.lineWidth = 1;
    ctx.strokeRect(14, 14, 1097, 389);
    ...
    const footerText = lang === 'en'
      ? '📍 Saime Backyard Cinema • 144 Ly Thai To, Hoi An • Table reservation recommended'
      : '📍 Rạp Chiếu Phim Saime • 144 Ly Thai To, Hội An • Khuyến khích quý khách đặt bàn lều trước';
    ctx.fillText(footerText, 1125 / 2, 403);
